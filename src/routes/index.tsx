...
const creations = [
  ["🎨", "Painéis e murais", "Deixe a sala mais bonita e acolhedora."],
  ["🔤", "Letras e números", "Apoie alfabetização, matemática e atividades visuais."],
  ["🐻", "Personagens", "Crie projetos infantis com elementos divertidos."],
  ["🎉", "Datas comemorativas", "Decore a sala para momentos especiais."],
  ["📚", "Cantinhos educativos", "Monte espaços de leitura, rotina e aprendizagem."],
  ["🌈", "Atividades criativas", "Tenha recursos para trabalhos manuais."],
];
...
function BuyCard({ kit, onTen }: { kit: typeof kits[number]; onTen: () => void }) {
  ...
  return <motion.article whileHover={{ y: -5 }} className={`relative flex flex-col rounded-3xl border p-5 ${kit.hot ? "border-violet-400 bg-violet-50 shadow-xl shadow-violet-100" : "border-slate-200 bg-white shadow-sm"}`}>
    ...
    <div className="mt-5 overflow-hidden rounded-2xl border border-slate-100 bg-violet-50"><div className="grid grid-cols-2 gap-2 p-3"><div className="flex h-20 items-center justify-center rounded-xl bg-white text-3xl">✂️</div><div className="flex h-20 items-center justify-center rounded-xl bg-white text-3xl">🎨</div></div><div className="bg-white px-4 py-4"><p className="text-sm text-slate-500">Você recebe</p><p className="text-2xl font-black">{kit.qty} Moldes</p></div></div>
...
function Index() {
  ...
  return <>
    ...
    <main className="min-h-screen overflow-x-hidden bg-[#fcfbff] text-slate-900">
      ...
      <section className="relative overflow-hidden bg-gradient-to-b from-violet-50 via-white to-[#fcfbff] py-12 sm:py-20"><div className="mx-auto grid max-w-6xl items-center gap-10 px-5 lg:grid-cols-2"><div className="text-center lg:text-left"><span className="rounded-full border border-violet-200 bg-white px-4 py-2 text-xs font-black uppercase text-violet-700 shadow-sm">✂️ Moldes digitais para EVA</span><h1 className="mt-6 text-4xl font-black leading-[1.04] sm:text-5xl md:text-6xl">Transforme EVA em <span className="text-violet-600">painéis incríveis</span> para sua sala</h1><p className="mx-auto mt-5 max-w-xl text-base leading-relaxed text-slate-600 sm:text-lg lg:mx-0">Tenha moldes prontos para criar personagens, letras, números, enfeites e atividades, sem precisar desenhar tudo do zero.</p><div className="mt-6 flex flex-wrap justify-center gap-2 lg:justify-start">{["Para professores","Feitos para EVA","Prontos para imprimir"].map(x => <span key={x} className="rounded-full border border-slate-200 bg-white px-3 py-2 text-xs font-bold text-slate-600">✓ {x}</span>)}</div><a href="#ofertas" className="mt-7 inline-flex min-h-14 w-full max-w-md items-center justify-center rounded-2xl bg-violet-600 px-7 text-sm font-black uppercase text-white shadow-lg shadow-violet-200 hover:bg-violet-700 sm:w-auto">Ver os kits</a><p className="mt-3 text-xs text-slate-400">Produto digital • Acesso após a compra • Pagamento seguro</p></div><div className="rounded-[2rem] border-8 border-white bg-violet-100 p-4 shadow-2xl"><div className="grid grid-cols-2 gap-3">{[["🐻","PERSONAGENS"],["🔤","LETRAS"],["🔢","NÚMEROS"],["🌈","DECORAÇÃO"]].map(([i,t])=><div key={t} className="flex h-36 flex-col items-center justify-center rounded-2xl bg-white shadow-sm sm:h-44"><span className="text-5xl">{i}</span><b className="mt-2 text-[10px] text-violet-700">{t}</b></div>)}</div><div className="-mt-5 mx-4 rounded-2xl bg-white p-4 text-center shadow-lg"><b>Do molde para o EVA ✂️</b><p className="mt-1 text-xs text-slate-500">Imprima, transfira, recorte e crie.</p></div></div></div></section>

      <section className="py-16"><div className="mx-auto max-w-6xl px-5"><Title eyebrow="Como funciona" title="Do molde à peça pronta" text="Um processo simples para você criar sem começar do zero."/><div className="mt-10 grid gap-4 md:grid-cols-3">{[["01","🖨️","Imprima","Escolha o molde e imprima."],["02","✂️","Passe para o EVA","Use o desenho como referência para contornar e recortar."],["03","🎨","Crie sua peça","Monte, decore e use na sala."]].map(([n,i,t,d])=><div key={n} className="rounded-3xl border border-slate-200 bg-white p-7 text-center shadow-sm"><span className="text-xs font-black text-violet-300">{n}</span><div className="mx-auto mt-3 flex size-16 items-center justify-center rounded-2xl bg-violet-50 text-3xl">{i}</div><h3 className="mt-5 text-lg font-black">{t}</h3><p className="mt-2 text-sm text-slate-500">{d}</p></div>)}</div></div></section>
      ...
      <section className="py-16"><div className="mx-auto max-w-6xl px-5"><Title eyebrow="Inspire-se" title="Veja o que você pode criar" text="Use os moldes em diferentes momentos da rotina escolar."/><div className="mt-10 grid gap-4 sm:grid-cols-2 lg:grid-cols-3">{creations.map(([i,t,d])=><motion.div whileHover={{ y:-3 }} key={t} className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm"><div className="flex size-14 items-center justify-center rounded-2xl bg-violet-50 text-3xl">{i}</div><h3 className="mt-5 text-xl font-black">{t}</h3><p className="mt-2 text-sm leading-relaxed text-slate-500">{d}</p></motion.div>)}</div></div></section>

      <section className="bg-slate-900 py-16 text-white"><div className="mx-auto grid max-w-6xl items-center gap-10 px-5 md:grid-cols-2"><div><span className="text-xs font-black uppercase tracking-[.18em] text-violet-300">Para quem vive a sala de aula</span><h2 className="mt-3 text-3xl font-black sm:text-4xl">Uma sala bonita sem complicar sua rotina</h2><p className="mt-4 text-sm leading-relaxed text-slate-300 sm:text-base">Prepare painéis, atividades e decoração partindo de moldes prontos e coloque sua criatividade no acabamento.</p><div className="mt-7 space-y-3 text-sm font-bold"><p>✓ Prepare painéis com mais rapidez</p><p>✓ Tenha ideias para novos projetos</p><p>✓ Crie materiais visuais para seus alunos</p><p>✓ Monte sua biblioteca digital de moldes</p></div></div><div className="grid grid-cols-2 gap-3 rounded-3xl bg-white/10 p-3">{["🐻 Personagens","🔤 Letras","🎨 Painéis","🌈 Decoração"].map(t=><div key={t} className="flex h-32 items-center justify-center rounded-2xl bg-white text-center text-sm font-black text-slate-900 sm:h-40">{t}</div>)}</div></div></section>
      ...
      <section className="bg-white py-20"><div className="mx-auto max-w-6xl px-5"><Title eyebrow="Depoimentos" title="Quem usa, recomenda" text="Área preparada para avaliações reais e fotos das peças feitas pelas clientes."/><div className="mt-10 grid gap-5 md:grid-cols-3">{["Professora de Educação Infantil","Professora do Ensino Fundamental","Professora / Artesã"].map(role=><article key={role} className="rounded-3xl border border-slate-200 bg-[#fcfbff] p-6 shadow-sm"><div className="text-amber-400">★★★★★</div><p className="mt-4 text-sm leading-relaxed text-slate-600">“Aqui entra o depoimento real da cliente sobre a praticidade e o resultado dos moldes.”</p><div className="mt-6 border-t border-slate-200 pt-4"><b>{role}</b><p className="mt-1 text-xs text-slate-400">Avaliação real a inserir</p></div></article>)}</div></div></section>
