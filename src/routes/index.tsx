...
function KitCard({ kit, onBuy10 }: { kit: (typeof kits)[number]; onBuy10: () => void }) {
  const buy = () => kit.qtd === 10 ? onBuy10() : undefined;
...
function UpsellModal({ open, onClose }: { open: boolean; onClose: () => void }) {
  ...
  return <div className="fixed inset-0 z-[100] flex items-center justify-center bg-slate-950/70 p-4 backdrop-blur-sm" role="dialog" aria-modal="true" aria-label="Oferta especial Kit 20 Peças">
    <motion.div initial={{ opacity: 0, scale: 0.94, y: 15 }} animate={{ opacity: 1, scale: 1, y: 0 }} className="relative w-full max-w-md overflow-hidden rounded-[2rem] bg-white shadow-2xl">
      ...
      <div className="p-6 text-center"><div className="rounded-2xl border-2 border-violet-100 bg-violet-50 p-5"><p className="text-xs font-black uppercase tracking-widest text-violet-600">Upgrade exclusivo</p><h3 className="mt-1 text-2xl font-black text-slate-900">Kit 20 Peças</h3><p className="mt-1 text-lg font-black text-violet-700">20 Moldes</p><div className="mt-3"><span className="text-sm text-slate-400 line-through">R$ 24,67</span><p className="text-4xl font-black text-violet-700">R$ 19,90</p></div><div className="mt-4 space-y-2 text-left text-sm text-slate-600"><p>✓ 20 moldes para EVA</p><p>✓ Mais variedade para seus projetos</p><p>✓ Condição exclusiva desta oferta</p><p>✓ Produto 100% digital</p></div></div><button onClick={() => { window.location.hash = "checkout"; onClose(); }} className="mt-5 w-full rounded-2xl bg-violet-600 px-5 py-4 text-sm font-black uppercase tracking-wide text-white shadow-lg shadow-violet-200 hover:bg-violet-700">Sim! Quero 20 Moldes por R$ 19,90</button><button onClick={onClose} className="mt-3 w-full py-2 text-xs font-bold text-slate-400 hover:text-slate-600">Não, prefiro continuar com o Kit 10</button><p className="mt-4 text-[0.65rem] text-slate-400">Oferta apresentada uma única vez ao escolher o Kit 10.</p></div>
    </motion.div>
  </div>;
}
...
function Index() {
  ...
  return <>
    ...
    <main className="overflow-x-hidden bg-[#fcfbff] font-[Inter,sans-serif] text-slate-900">
      <section className="relative overflow-hidden border-b border-violet-100 bg-gradient-to-b from-violet-50 via-white to-[#fcfbff] py-10 sm:py-16"><div className="pointer-events-none absolute -right-32 -top-32 size-80 rounded-full bg-violet-200/50 blur-3xl" /><div className="pointer-events-none absolute -left-32 bottom-0 size-72 rounded-full bg-fuchsia-100/70 blur-3xl" /><div className="container relative mx-auto max-w-6xl px-5"><div className="grid items-center gap-9 lg:grid-cols-[1.05fr_0.95fr]"><div className="text-center lg:text-left"><span className="inline-flex rounded-full border border-violet-200 bg-white px-4 py-2 text-[0.68rem] font-extrabold uppercase tracking-[0.15em] text-violet-700 shadow-sm">✂️ Moldes digitais para EVA</span><h1 className="mt-5 text-4xl font-black leading-[1.05] tracking-tight sm:text-5xl md:text-6xl">Transforme EVA em <span className="text-violet-600">painéis incríveis</span> para sua sala de aula</h1><p className="mx-auto mt-5 max-w-2xl text-base leading-relaxed text-slate-600 sm:text-lg lg:mx-0">Tenha moldes prontos para criar personagens, letras, números, enfeites e atividades — <strong>sem precisar desenhar tudo do zero.</strong></p><div className="mt-6 flex flex-wrap justify-center gap-2.5 lg:justify-start">{["Para professores", "Feitos para EVA", "Prontos para imprimir"].map(x => <span key={x} className="rounded-full border border-slate-200 bg-white px-4 py-2 text-xs font-bold text-slate-600 shadow-sm">✓ {x}</span>)}</div><a href="#ofertas" className="mt-7 inline-flex min-h-14 w-full max-w-md items-center justify-center rounded-2xl bg-violet-600 px-7 text-base font-black uppercase tracking-wide text-white shadow-lg shadow-violet-200 transition hover:-translate-y-0.5 hover:bg-violet-700 sm:w-auto">Ver os kits de peças</a><p className="mt-3 text-xs text-slate-400">Produto 100% digital • Acesso após a compra • Pagamento seguro</p></div><div className="relative mx-auto w-full max-w-lg"><div className="overflow-hidden rounded-[2rem] border border-white bg-white p-2 shadow-2xl shadow-violet-100"><div className="grid grid-cols-2 gap-2 rounded-[1.5rem] bg-violet-50 p-3">{['PERSONAGENS','LETRAS','NÚMEROS','DECORAÇÃO'].map((t,i)=><div key={t} className="flex h-32 items-center justify-center rounded-2xl bg-white text-center shadow-sm sm:h-40"><div><div className="text-4xl">{['🐻','🔤','🔢','🌈'][i]}</div><p className="mt-2 text-[0.65rem] font-black text-violet-700">{t}</p></div></div>)}</div><div className="relative mx-4 -mt-7 rounded-2xl border border-slate-100 bg-white p-4 shadow-lg"><p className="text-sm font-black">Do molde para o EVA ✂️</p><p className="mt-1 text-xs leading-relaxed text-slate-500">Imprima, transfira para o EVA e crie suas peças para a sala.</p></div></div></div></div></div></section>
      <section className="py-14 sm:py-18"><div className="container mx-auto max-w-6xl px-5"><SectionTitle eyebrow="Veja a ideia" title="Do molde à peça pronta" text="A proposta é simples: você recebe o desenho, imprime e usa como base para produzir suas criações em EVA." /><div className="mt-10 grid gap-4 md:grid-cols-3">{[['01','🖨️','Imprima','Escolha o molde e imprima no tamanho que precisar.'],['02','✂️','Passe para o EVA','Use o molde como referência para contornar e recortar.'],['03','🎨','Crie sua peça','Monte, decore e use na sua sala de aula.']].map(([num,icon,title,text])=><div key={num} className="relative rounded-3xl border border-slate-200 bg-white p-6 text-center shadow-sm"><span className="absolute right-4 top-4 text-xs font-black text-violet-200">{num}</span><div className="mx-auto flex size-16 items-center justify-center rounded-2xl bg-violet-50 text-3xl">{icon}</div><h3 className="mt-5 text-lg font-black">{title}</h3><p className="mt-2 text-sm leading-relaxed text-slate-500">{text}</p></div>)}</div></div></section>
