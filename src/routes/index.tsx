...
export const Route = createFileRoute("/")({
  ...
  head: () => ({
    meta: [
      { title: "Moldes de EVA para Professores | Kits 10, 20, 30 e 50 Peças" },
      { name: "description", content: "Kits de peças digitais para professores criarem painéis, murais, decoração e atividades em EVA." },
    ],
  }),
});

const kits = [
  { nome: "Kit 10 Peças", preco: "14,67", antigo: "29,90", destaque: false, texto: "Para começar" },
  { nome: "Kit 20 Peças", preco: "24,67", antigo: "59,90", destaque: true, texto: "Mais escolhido" },
  { nome: "Kit 30 Peças", preco: "34,67", antigo: "89,90", destaque: false, texto: "Mais variedade" },
  { nome: "Kit 50 Peças", preco: "49,67", antigo: "149,90", destaque: false, texto: "Coleção completa" },
];

const beneficios = [
  ["✂️", "Feitos para EVA", "Peças digitais pensadas para recortar, montar e transformar em trabalhos de EVA."],
  ...
  ["♻️", "Use novamente", "Guarde os arquivos e reutilize as peças nos seus próximos projetos."],
  ["📱", "100% digital", "Acesse pelo celular ou computador e mantenha sua coleção organizada."],
];

const depoimentos = [
  ["Professora de Educação Infantil", "Espaço reservado para um depoimento real de uma professora sobre a praticidade das peças."],
...
const duvidas = [
  ["As peças são para EVA?", "Sim. A coleção é direcionada para trabalhos com EVA, especialmente para professores que produzem peças para a sala de aula."],
  ["É um produto físico?", "Não. É um produto digital. Nenhum item físico será enviado."],
  ["Como uso as peças?", "Você recebe os arquivos digitais, imprime e utiliza os desenhos como base para cortar e montar suas peças em EVA."],
...
function Index() {
  return <>
    ...
    <div className="sticky top-0 z-50 border-b border-violet-100 bg-white/95 py-2.5 text-center text-[0.68rem] font-extrabold uppercase tracking-wide text-violet-700 backdrop-blur sm:text-xs">📚 Peças de EVA para professores • Acesso digital</div>
    <main className="overflow-x-hidden bg-[#fcfbff] font-[Inter,sans-serif] text-slate-900">
      <section className="relative overflow-hidden border-b border-violet-100 bg-gradient-to-b from-violet-50 via-white to-[#fcfbff] py-12 sm:py-20">
        ...
        <div className="container relative mx-auto max-w-6xl px-5"><div className="grid items-center gap-10 lg:grid-cols-[1.05fr_0.95fr]">
          <div className="text-center lg:text-left"><span className="inline-flex rounded-full border border-violet-200 bg-white px-4 py-2 text-[0.68rem] font-extrabold uppercase tracking-[0.15em] text-violet-700 shadow-sm">✂️ Coleção de peças para EVA</span>
            <h1 className="mt-5 text-4xl font-black leading-[1.05] tracking-tight sm:text-5xl md:text-6xl">Crie uma sala de aula mais bonita <span className="text-violet-600">sem perder horas criando peças</span></h1>
            <p className="mx-auto mt-6 max-w-2xl text-base leading-relaxed text-slate-600 sm:text-lg lg:mx-0">Kits de peças digitais para professores que trabalham com EVA e querem preparar painéis, murais, decoração e atividades de forma prática.</p>
            <div className="mt-7 flex flex-wrap justify-center gap-2.5 lg:justify-start">{["Para professores", "Peças para EVA", "Prontas para imprimir"].map(x => <span key={x} className="rounded-full border border-slate-200 bg-white px-4 py-2 text-xs font-bold text-slate-600 shadow-sm">✓ {x}</span>)}</div>
            <a href="#ofertas" className="mt-8 inline-flex min-h-14 w-full max-w-md items-center justify-center rounded-2xl bg-violet-600 px-7 text-base font-black uppercase tracking-wide text-white shadow-lg shadow-violet-200 transition hover:-translate-y-0.5 hover:bg-violet-700 sm:w-auto">Ver os kits de peças</a>
            <p className="mt-4 text-xs text-slate-400">Produto 100% digital • Pagamento seguro</p>
          </div>
          <div className="relative mx-auto w-full max-w-lg"><div className="overflow-hidden rounded-[2rem] border border-white bg-white p-2 shadow-2xl shadow-violet-100"><img src="https://images.unsplash.com/photo-1588072432836-e10032774350?auto=format&fit=crop&w=1200&q=85" alt="Professora em ambiente de sala de aula" className="h-[360px] w-full rounded-[1.5rem] object-cover sm:h-[430px]" /><div className="relative mx-4 -mt-12 rounded-2xl border border-slate-100 bg-white p-4 shadow-lg"><p className="text-sm font-black">Transforme ideias em peças de EVA</p><p className="mt-1 text-xs leading-relaxed text-slate-500">Imprima a peça, passe para o EVA e monte suas criações para a sala.</p></div></div></div>
        </div></div>
      </section>

      <section className="py-16 sm:py-20"><div className="container mx-auto max-w-6xl px-5"><SectionTitle eyebrow="Feito para sua rotina" title="Seu trabalho com EVA começa pela peça certa" text="Tenha uma coleção digital para facilitar os momentos em que você precisa preparar uma sala bonita, uma atividade ou um painel especial." /><div className="mt-10 grid gap-4 sm:grid-cols-2 lg:grid-cols-3">{beneficios.map(([icon,title,text],i)=><motion.div key={title} initial={{opacity:0,y:15}} whileInView={{opacity:1,y:0}} viewport={{once:true}} transition={{delay:i*.04}} className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm"><span className="flex size-11 items-center justify-center rounded-xl bg-violet-50 text-xl">{icon}</span><h3 className="mt-4 text-lg font-black">{title}</h3><p className="mt-2 text-sm leading-relaxed text-slate-500">{text}</p></motion.div>)}</div></div></section>

      <section className="border-y border-violet-100 bg-violet-50/60 py-16"><div className="container mx-auto max-w-6xl px-5"><div className="grid items-center gap-8 md:grid-cols-2"><div className="overflow-hidden rounded-3xl bg-white p-2 shadow-xl shadow-violet-100"><img src="https://images.unsplash.com/photo-1516627145497-ae6968895b74?auto=format&fit=crop&w=1000&q=85" alt="Ambiente educativo" className="h-72 w-full rounded-2xl object-cover md:h-80" /></div><div><span className="text-xs font-extrabold uppercase tracking-[0.18em] text-violet-600">Para quem vive a sala de aula</span><h2 className="mt-3 text-3xl font-black leading-tight sm:text-4xl">Mais criatividade para seus projetos</h2><p className="mt-4 text-sm leading-relaxed text-slate-600 sm:text-base">Use as peças para criar personagens, letras, números e elementos decorativos em EVA.</p><div className="mt-6 space-y-3 text-sm font-bold text-slate-700"><p>✓ Murais e painéis temáticos</p><p>✓ Decoração para datas comemorativas</p><p>✓ Cantinhos educativos</p><p>✓ Atividades e projetos com os alunos</p></div></div></div></div></section>

      <section id="ofertas" className="scroll-mt-16 py-20"><div className="container mx-auto max-w-7xl px-5"><SectionTitle eyebrow="Escolha seu kit" title="Quantas peças você quer ter?" text="Todos os kits são digitais e pensados para facilitar seu trabalho com EVA." /><div className="mt-12 grid gap-5 sm:grid-cols-2 lg:grid-cols-4">{kits.map(kit=><KitCard key={kit.nome} kit={kit}/>)}</div></div></section>
      ...
      <section className="bg-slate-900 py-20 text-white"><div className="container mx-auto max-w-5xl px-5 text-center"><span className="text-xs font-extrabold uppercase tracking-[0.18em] text-violet-300">Seu próximo projeto começa aqui</span><h2 className="mt-3 text-3xl font-black sm:text-4xl">Tenha suas peças de EVA sempre à mão</h2><p className="mx-auto mt-4 max-w-2xl text-sm leading-relaxed text-slate-300 sm:text-base">Escolha seu kit e tenha uma coleção digital para preparar suas próximas criações.</p><a href="#ofertas" className="mt-8 inline-flex min-h-14 rounded-2xl bg-violet-500 px-8 py-4 text-sm font-black uppercase tracking-wide text-white shadow-lg shadow-violet-900/30 transition hover:bg-violet-400">Escolher meu kit</a></div></section>
      ...
      <footer className="border-t border-slate-200 bg-white py-8 text-center text-xs text-slate-400"><p>© {new Date().getFullYear()} • Moldes e peças digitais para EVA</p></footer>
    </main>
  </>;
}
